...
const replacePlaceholder = (documentContent, jsonContent) => {
    ...
    const processTagValue = (value) => {
        if (Array.isArray(value)) {
            return value.map((item, index) => {
                if (typeof item === 'string') {
                    return { text: `${index + 1}. ${item}`, highlighted: false };
                } else if (typeof item === 'object' && item.text) {
                    return { text: `${index + 1}. ${item.text}`, highlighted: item.highlighted || false };
                }
                return { text: '', highlighted: false };
            });
        } else if (typeof value === 'object' && value.text) {
            return [{ text: value.text, highlighted: value.highlighted || false }];
        }
    ...
    select("//w:sdt", doc).forEach(node => {
        ...
        if (tagNode) {
            ...
            if (tags.hasOwnProperty(tagName)) {
                const textNodes = select('.//w:t', node);
                if (textNodes.length > 0) {
                    const processedValues = processTagValue(tags[tagName]);

                    // Clear existing text nodes
                    textNodes.forEach((textNode, index) => {
                        if (index === 0) {
                            textNode.textContent = processedValues[0].text;
                        } else {
                            textNode.parentNode.removeChild(textNode);
                        }
                    });
                    ...
                    if (processedValues[0].highlighted) {
                        ...
                        highlightNode.setAttribute('w:val', 'yellow'); // Change to desired highlight color
                        runProperties.appendChild(highlightNode);
                        textNodes[0].parentNode.insertBefore(runProperties, textNodes[0]);
                    }
                }
            }
        }
