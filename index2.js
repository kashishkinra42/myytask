...
const replacePlaceholder = (documentContent, jsonContent) => {
    ...
    const createNumberedParagraphs = (doc, value) => {
        return value.map((item, index) => {
            ...
            t.textContent = `${index + 1}. ${typeof item === 'string' ? item : item.text}`;
            r.appendChild(t);
            p.appendChild(r);
            return p;
        });
    ...
    select("//w:sdt", doc).forEach(node => {
        ...
        if(tagNode){
            ...
            if(tags.hasOwnProperty(tagName)){
                ...
                const textNodes = select('.//w:t', node);
                if(textNodes.length > 0){
                    const parentNode = textNodes[0].parentNode.parentNode;
                    const processedValue = createNumberedParagraphs(doc, tagValue);
                    processedValue.forEach(p => {
                        parentNode.parentNode.insertBefore(p, parentNode);
                    });
                    parentNode.parentNode.removeChild(parentNode); // Remove the original placeholder
                }
            }
        }
    });
